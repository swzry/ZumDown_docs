...
另外Superblock支持嵌套，但嵌套的语法大相径庭，请参阅： Superblock嵌套说明

### Superspan

这是一种可扩展的行内高级元素，基本格式为`[内容]{Superspan控制语句}`，前后需加上空白符与文本隔开。Superspan控制语句语法方面和Superblock相同，也是`类型:样式参数`。

示例：

```code:zumdown
	这是一段文字，其中这是 [红色的]{color:red} ，这是 [蓝色的]{color:blue} ，这是 [#9900CC色]{color:#9900CC} 。
```
